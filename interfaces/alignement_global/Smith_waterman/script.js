	...
	for(i = 0; i < letter2.length; i++)
	{
		...
		for(j = 0; j < letter1.length; j++)
		{
			if(letter1[j] == "#" || letter2[i] == "#")
				str += "<td><input id=\"" + letter1[j] + "_" + letter2[i] + "\" type='text' value='-6' /></td>";
			else if(letter1[j] == letter2[i])
				str += "<td><input id=\"" + letter1[j] + "_" + letter2[i] + "\" type='text' value='4' /></td>";
			else
				str += "<td><input id=\"" + letter1[j] + "_" + letter2[i] + "\" type='text' value='-5' /></td>";
		}
...
function valueScore(m,n)
{
	...
	else
	{
		...
		{
			score[m][n] = Math.max(0, valueScore(m-1,n) + cout(m, 0), valueScore(m,n-1) + cout(0, n), valueScore(m-1,n-1) + cout(m,n))
		}
...
function findAlignement(m,n, alignement)
{
	...
	{
		if(score[m][n] == valueScore(m-1,n) + cout(m, 0))
		{
			alignement.alignement.push([m,n]);
			findAlignement(m-1, n, alignement);
			document.getElementById("score_" + m + "_" + n).style = "background-color:red";
		}
		else if(score[m][n] == valueScore(m,n-1) + cout(0, n))
		{
			alignement.alignement.push([m,n]);
